...
__global__ void d_tumble(int no_of_colloid, point *d_ra, point *d_pos, point len){
	d_img(&d_ra[blockIdx.x], d_pos[blockIdx.x] - d_ra[blockIdx.x].random(point(0,0,0),len),len);
	d_ra[blockIdx.x] = d_ra[blockIdx.x]/sqrt((d_ra[blockIdx.x]*d_ra[blockIdx.x]).sum());
}
...
void tumble(){
   ...
   /* 
    ...
    cudaMemcpy(d_ra, ra, (no_of_colloid + 2)*sizeof(point), cudaMemcpyHostToDevice);
  	...
  	d_tumble<<<no_of_colloid + 1,1>>>(no_of_colloid, d_ra, d_pos, len);
