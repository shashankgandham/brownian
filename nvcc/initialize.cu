...
void initialize() {
	...
	for(int i = 0; i < 8; i++) {
		if(i < 5) cudaMallocManaged(ipointers[i], (isize[i>0] + 2)*sizeof(int));
				  cudaMallocManaged(ppointers, (psize[i>1] + 2)*sizeof(point));
	}
...
void initialize_colloid() {
	d_initialize_colloid<<<1, 1>>> (pos_colloid, vel_colloid, ang_vel_colloid, sig_colloid, kbt1, 
		I_colloid, mass_colloid, no_of_colloid, len); 
}
void initialize_fluid() {
	d_initialize_fluid<<<1, 1>>> (pos_fl, vel_fl, pos_colloid, kbt, mass_fl, sigma, no_of_fluid, no_of_colloid, len); 
}