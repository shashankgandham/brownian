...
__global__ void d_rotation_mpcd(point *vel_fl, point *pos_fl, point *cell_vel, int *fluid_no, int **cell_part, int no_of_fluid, 
		point len, double kbt, double mass_fl, int *iv, int *seed, int *idum, int *iy) {
	int k;
	double r[4], ir[4], theta, phi, rho, ct, st, ict;
	point del_v, rot[4], temp;
...
void rotation_mpcd() {
	...
	d_rotation_mpcd<<<1, 1>>> (vel_fl, pos_fl, cell_vel, fluid_no, cell_part, no_of_fluid, len, kbt, mass_fl, iv, seed, idum, iy);
