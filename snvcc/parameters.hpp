...
extern int n, niter, file, nbin, no_of_fluid, maxpart, no_of_colloid, nbox, **nbr, **up_nbr, *cnt, *up_cnt, nn;
...
extern int seed, ntab, *iv;
double ran() {
	int im1 = 2147483563, im2 = 2147483399, ia1 = 40014, ia2 = 40692, iq1 = 53668, iq2 = 52774, iy, j, k;
	int imm = im1 - 1, ir1 = 12211, ir2 = 3791, ntab = 32, ndiv = 1 + imm/ntab, idum = 123456789;
...
struct point {
	...
	point random(point dec = point(0, 0 ,0), point mul = point(1, 1, 1)) {
		x = ran(); y = ran(); z = ran();
		*this = (*this)*mul - dec;
		return *this;
	}
...
extern point *pos_colloid, *pos_fl, *vel_colloid, *vel_fl, *ang_vel_colloid, *f, *old_force, *ra, len, temp;
...
/*__device__ void round(point *c, point a) {
    *c = point((int)(a.x + 0.5), (int)(a.y + 0.5), (int)(a.z + 0.5));
}
__device__ void mod(point *a, point b) {
    point temp;
    round(&temp, (*a - b/2)/b);
    *a = *a - b*temp;
}
__device__ void img(point *a, point b) {
    point temp;
    round(&temp, ((*a)/b));
    *a = *a - b*temp;
}*/
