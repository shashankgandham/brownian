...
__global__ void d_fluid_colloid_collision(int *no_neigh, point *pos_colloid, point *pos_fl, 
	...
	if(j <= no_of_colloid) {
		...
		if(i <= 10000) vc[j][i] = om[j][i] = point(0, 0, 0);
		if(i <= no_neigh[j]) {
			int l = neigh_fl[j][i];
...
__global__ void d_dump(point *dump_vel_fl, point *vel_fl, double *potential_colloid, int no_of_fluid) {
	int i = blockIdx.x*blockDim.x + threadIdx.x + 1;
	if(i <= no_of_fluid) dump_vel_fl[i] = vel_fl[i];
	if(i == 1) *potential_colloid = 0;
}

__global__ void reduce(point **vc, point **om, int *no_neigh, int no_of_colloid) {

	int i = blockIdx.x*blockDim.x + threadIdx.x + 1;
	int j = (i%no_of_colloid) + (!(i%no_of_colloid))*no_of_colloid;
	if(i <= no_of_colloid) { 
		vc[j][0] = thrust::reduce(thrust::device, vc[j], vc[j] + no_neigh[j] + 1, point(0, 0, 0), add_point());
	}
	else if(i <= 2*no_of_colloid) { 
...
void fluid_colloid_collision() {
	...
	blk = dim3((2*no_of_colloid + thr.x -1)/thr.x);
	reduce<<<blk, thr>>>(vc, om, no_neigh, no_of_colloid);
	blk = dim3((2*no_of_colloid + thr.x -1)/thr.x);
