subroutine run
	...
	real*8 ::cos_b1,cos_b2,cos_b3,sin_b1,sin_b2,sin_b3
	real*8 ::b1,b2,b3,bb1,bb2,bb3,ran1,v01,v02,v03
	real*8 ::m11,m12,m13,m21,m22,m23,m31,m32,m33
	real*8 :: vector_x,vector_y,vector_z,vector,dot
	real*8 ::velx1,vely1,velz1,velx2,vely2,velz2,velx12,vely12,velz12
	real*8 :: vv, theta,change,ke_before,ke_after,xx,vv1,vv2
	real*8 ::up_velx2,up_vely2,up_velz2,aaa,delx,dely,delz
	real*8 ::dumx,dumy,dumz

!	!!The bacterium has to tumble first to get the direction of new velocity the tumble angle is random in (0,2pi)!!
...
!	if(tumble.ne.0)then
	...
	do i=1,no_of_colloid
		...
		vel_colloid(3*i-2)=vel_colloid(3*i-2)+v0*ra(3*i-2) !-v0*old_ra(3*i-2)
		vel_colloid(3*i-1)=vel_colloid(3*i-1)+v0*ra(3*i-1) !-v0*old_ra(3*i-1)
		vel_colloid(3*i)=vel_colloid(3*i)+v0*ra(3*i) !-v0*old_ra(3*i)

		delx=vel_colloid(3*i-2)-dumx
		dely=vel_colloid(3*i-1)-dumy
		delz=vel_colloid(3*i)-dumz

!		!!the added momenta has to be taken out from the neighbouring fluid to conserve momentum!!
		cnt(i)=0;up_cnt(i)=0
